...
def _cua_call(tool: str, params: dict) -> dict | None:
    ...
    if _CUA_CLIENT_MODE == "http":
        try:
            ...
            resp = urllib.request.urlopen(r, timeout=30)  # noqa: S310 - local bridge
...
def launch_app():
    ...
    for attempt in range(MAX_RETRY):
        try:
            resp = urllib.request.urlopen(f"{BACKEND_URL}{HEALTH_PATH}", timeout=5)  # noqa: S310 - local bridge
            if resp.status == 200:
...
def check_feature_route():
    try:
        resp = urllib.request.urlopen(f"{BACKEND_URL}{FEATURE_PATH}", timeout=5)  # noqa: S310 - local bridge
...
def check_diagnostics():
    try:
        resp = urllib.request.urlopen(f"{BACKEND_URL}{DIAGNOSTICS_PATH}", timeout=5)  # noqa: S310 - local bridge
...
def nav_click_through(output_dir: str):
    ...
    for label, expected_header in nav_routes:
        try:
            idx = next((i for i, (ln, _) in enumerate(nav_routes) if ln == label), 0)
