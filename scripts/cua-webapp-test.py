...
def wait_backend():
    ...
    while time.time() < deadline:
        try:
            r = urllib.request.urlopen(url, timeout=3)  # noqa: S310 - local bridge
            if r.status == 200:
...
def check_diagnostics():
    try:
        r = urllib.request.urlopen(f"{BACKEND_URL}/api/v1/diagnostics", timeout=5)  # noqa: S310 - local bridge
