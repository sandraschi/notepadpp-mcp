...
import { FileStack, HardDrive, RefreshCw } from "lucide-react";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Live data from <code className="text-slate-300">GET /api/editor</code>
          . On-disk stats appear when the Notepad++ title contains a real path
          (e.g.{" "}
          <code className="text-slate-300">D:\work\file.py - Notepad++</code>).
        </p>
      </div>
      ...
      <div className="flex gap-2">
        ...
        >
          <RefreshCw
            className={`h-4 w-4 mr-2 ${editor.isFetching ? "animate-spin" : ""}`}
          />
          Refresh snapshot
        </Button>
      </div>
            ...
            <>
              <div className="grid gap-3 md:grid-cols-2 text-sm">
                <div>
                  ...
                  <p className="text-slate-100 font-medium break-all">
                    {String(
                      ed.active_file_display_name ?? ed.active_file_hint ?? "—",
                    )}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-slate-100">
                    {ed.is_modified_hint === true ? "Yes" : "No"}
                  </p>
                </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-emerald-400/90 font-mono text-sm break-all">
                    {ed.resolved_path != null
                      ? String(ed.resolved_path)
                      : "— (save to a path or open a file with full path in title)"}
                  </p>
                </div>
              </div>
              ...
              <div>
                <p className="text-slate-500 text-xs uppercase mb-2">
                  Per-file stats (when path resolves)
                </p>
                <pre className="text-xs p-4 rounded-md border border-slate-800 bg-slate-900/70 text-slate-300 overflow-x-auto whitespace-pre-wrap">
                  {fs
                    ? fmtStats(fs)
                    : "No resolvable path — stats unavailable."}
                </pre>
              </div>
            </>
          )}
      ...
      <Card className="border-slate-800 bg-slate-950/50">
        ...
        <CardContent className="space-y-3">
          <p className="text-slate-400 text-sm">
            Calls{" "}
            <code className="text-slate-300">GET /api/file/stats?path=…</code> —
            useful for any file on disk, not only the active tab.
          </p>
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <Button
              type="button"
              onClick={() =>
                manualPath.trim() && manualStats.mutate(manualPath.trim())
              }
          ...
          {manualStats.isError && (
            <p className="text-red-400 text-sm">
              {(manualStats.error as Error).message}
            </p>
          )}
        </CardContent>
      </Card>
