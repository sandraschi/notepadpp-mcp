...
app = FastMCP("my-server", version="1.0.0", description="My MCP server with advanced features")
...
async def sample_workflow(iterations: int = 5) -> dict:
    ...
    return {"success": True, "samples_processed": iterations, "results": results}
...
async def test_sample_workflow():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            result = await session.call_tool("sample_workflow", arguments={"iterations": 3})
...
```python
@app.tool()
async def iterative_refinement(ctx, prompt: str, max_iterations: int = 5, quality_threshold: float = 0.8) -> dict:
    ...
    return {"final_result": best_result, "quality_score": best_score, "iterations_used": iteration + 1}
```

### **🤝 Cooperative Patterns**
```python
@app.tool()
async def collaborative_analysis(ctx, data_source: str, analysis_type: str, collaborators: list[str] = None) -> dict:
    ...
    if collaborators:
        enhanced_data = await collaborative_enhancement(processed_data, collaborators)
    ...
    return {"result": final_result, "cooperative_mode": bool(collaborators), "phases_completed": 4}
```
...
async def robust_operation(param: str) -> dict:
    try:
        ...
        logger.error(
            "Invalid parameter",
            extra={"operation": "robust_operation", "error": str(e), "error_type": type(e).__name__},
        )
        return {"error": "Invalid parameter", "recovery": "Check input format"}
    except Exception as e:
        logger.error(
            "Unexpected error",
            extra={"operation": "robust_operation", "error": str(e), "traceback": traceback.format_exc()},
        )
...
async def managed_session():
    session = ClientSession(
        connector=aiohttp.TCPConnector(limit=100, ttl_dns_cache=30), timeout=aiohttp.ClientTimeout(total=30)
    )
    try:
        yield session
    finally:
